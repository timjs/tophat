...
\end{enumerate*}
For this example we assume that the host language has lists and four functions on them: all, any, intersect, and difference. \fixme{Alejandro: small description may be needed} %nico: of the functions
...
\begin{figure}[b]
  ...
  \caption{
    ...
    The first user can not pick seats \smallcaps{1b} and \smallcaps{1c} any more.
...
A flight booking starts with an interactive task \TS{enter (List Passenger)}
...
\end{TASK}
\fixme{Alejandro: At this point I wonder, what is $\Enter$}
...
and displays the end result using the \TS{edit}-construct.\fixme{Alejandro: Give \TS{edit} a proper name?}
\begin{TASK}
