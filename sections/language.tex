...
The host language is a simply typed lambda calculus, extended with some basic types and \ML-style references.\fixme{Alejandro: why references?}
...
\end{figure}
\fixme{Alejandro about l: different syntax from vars?}
\label{sub:notation}
...
This means they are considered to be in normal form for the host language, even if their subexpressions are not in normal form.
...
\fixme{Say someting about why we need beta, and use beta in the acutal typing rule}
\fixme{Alejandro: Is there anything surprising? Say what}
\begin{figure}[h]
  \small
  \begin{gather*}
...
\paragraph{Internal and external step $(t \Then e, t \Next e)$}\fixme{Alejandro: The naming is confusing to me}
\label{sub:steps}
...
This means users cannot work on the branches of an external choice before picking one. \fixme{Alejandro: Is this actually implied?}
