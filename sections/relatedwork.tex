...
\paragraph{iTasks}
\fixme{Rinus: Waarom geen korte opsomming van iTask features die niet zijn meegenomen in TopHat?}
...
\fixme{Rinus: Volgens mij is er wel degelijk een formele semantiek, omdat heel iTask wordt uitgelegd in termen van standaard Clean functies,
en Clean heeft een semantiek (graaf herschrijf systeem).
Dat de beschrijving onhandig (wellicht te low level is) is voor het formeel redeneren over iTask programma's is een beter argument.}
\fixme{Alejandro: What is missing from \TOPHAT to describe i or m Tasks}
