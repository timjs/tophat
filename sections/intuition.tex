...
This motivates the combinators step ($\Then$, $\Next$), parallel ($\And$), and choice ($\Or$, $\Xor$).

\begin{example}[Breakfast]
\label{exm:breakfast}

The following program shows the different collaboration operators in action.
Users have a choice whether they want tea or coffee.
They always get an egg.
The drink and the food are prepared in parallel.
When drink and food are prepared, users can step to eating the result.
...
\Cref{exm:breakfast} demonstrates how tasks can be parameterized by other tasks: makeBreakfast is a collaboration pattern that always works the same way, regardless of which food and drink are being prepared.
