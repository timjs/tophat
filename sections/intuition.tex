...
\begin{TASK}
  let twice : Task $\tau$ -> Task $\tau$ = \t : Task $\tau$.
    t >>? \x : $\tau$. t
\end{TASK}
By using the step combinator $\When$ we specify that the given task \TS{t} should be performed a second time.
The result \TS{x} from the first run is ignored.
...
Editors form the leaves of such trees \todo{which trees?}.
